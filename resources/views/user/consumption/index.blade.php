    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg h-full">
                <x-search class="w-1/2" required="0"/>
                            ...
                            @foreach($users as $user)
                                <x-table.row>
                                    ...
                                    <x-table.row-item id="user-{{$user->id}}-remaining">
                                        <x-badge :content="number_format($user->getRemainingFuelQuota(), 2)"
                                                 type="primary"/>
                                    </x-table.row-item>
                                    @can('admin')
                                        <x-table.row-item>
                                            @if ($user->getRemainingFuelQuota() > 0)
                                                <a href="#!"
                                                   data="{{$user->id}}"
        ...
        <script>
            $(document).ready(function () {
                ...
                function loadSearchResults(searchVal) {
                    let route = "{{route('admin.users.withCurrentPlanFuelConsumption')}}" + "?search=" + searchVal;
                ...
                function sendConsumedPostRequest(userId, addedAmount) {
                    ...
                    $.ajax({
                        ...
                        success: function () {
                            ...
                            $('input').val('');
                            clearErrors()
                            const alertWrapperElm = $('#alert-wrapper');
                            alertWrapperElm.empty();
                            alertWrapperElm.append(`<x-alert  type='success'
